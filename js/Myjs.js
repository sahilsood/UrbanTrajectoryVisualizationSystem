...
map.on('draw:created', function (e) {
	...
	if (type === 'rectangle') {
		...
		DrawRS(result);
		//document.getElementById("demo").innerHTML = result[0].streetnames;	
		if (result){
			//window.q=result;
			window.localStorage.setItem('user', result[0].streetnames);
			window.localStorage.setItem('scatter', JSON.stringify(result));
			document.getElementById("word").innerHTML = 'Word Cloud';
			document.getElementById("sankey").innerHTML = 'Sankey Diagram';
			document.getElementById("scatter").innerHTML = 'Scatter Matrix';
			//$("#word a").attr("href", '/js/cloud.html' + "?var1=" + result);
		}
		});
	}
